...
function TechConfig() {
    ...
    useEffect(() => {
        fetch(`http://localhost:9090/Technician/${id}`)
            .then((res) => res.json())
            .then((data) => {
                setDevice(data);
                console.log(data);
        ...
        if (!available || device.filter(item => item.status === false).length === 0) {
            ...
            return device.filter(item => item.status === false).map(item =>
                <div key={item.index} className="bg-white flex px-8 py-7 space-y-4 flex-col w-[35%] overflow-hidden  items-center rounded-md">
                    ...
                    <div>
                        <Link to={`/Technician`}>  <Button className="bg-blue-900 hover:bg-blue-950 text-white"  onClick={()=>handleClick(item.id)}>Connect it</Button></Link>
                    </div>
                </div>)
