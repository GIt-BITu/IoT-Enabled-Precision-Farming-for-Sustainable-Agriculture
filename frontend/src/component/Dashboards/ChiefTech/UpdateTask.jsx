        ...
        <div className="bg-blue-950 w-[calc(100%-275px)] flex justify-center items-center rounded-md p-2 ">
            <div className="bg-white shadow rounded-md p-5 space-y-4 space-x-3 w-[50%] ">
                ...
                <div className="flex justify-center h-full w-full items-center ">
                    <Link to={`/chiefTechnician`}><Button
                        className={` text-white bg-slate-500 hover:bg-slate-600 block ${appear && 'hidden'}`}>return</Button></Link>


                    <div className={`${!appear && 'hidden'} flex-col h-full gap-5 items-center`}>
                        <div className="flex gap-5 flex-col justify-around items-center ">
                            <input type="text" required="required"
                                   className="border-2 p-2 rounded-md border-black shadow" name="title" value={title}
                                   onChange={(e) => setTitle(e.target.value)} placeholder="title"/>
                            <textarea required="required" className="p-2 resize-none h-52" name="description"
                                      value={description} onChange={(e) => setDescription(e.target.value)}
                                      placeholder="Description"></textarea>
                        </div>
                        <div className="flex justify-around items-center mt-4">
                            <Link to={`/chiefTechnician`}><Button
                                className="text-white bg-slate-500 hover:bg-slate-600">return</Button></Link>
                            <Link to={`/chiefTechnician`}><Button onClick={handleClick}
                                                                  className="text-white bg-blue-900 hover:bg-blue-950">Finish
                                it</Button></Link>
                        </div>
                    </div>
