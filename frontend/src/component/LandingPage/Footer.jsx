...
function Footer() {
    const Flist = List1.map((item, index) => (
        <li key={index}>
            <Link to={item.path}>
    ...
    const Slist = List2.map((item, index) => (
        <li key={index}>
            <Link to={item.path}>
                <span className="text-white">{item.name}</span>
            </Link>
    ...
    const Media = SocialM.map((item, index) => (
        <li key={index}>
            <Link to={item.path}>
        ...
        <footer className="justify-center items-center">
            <div className="flex items-center justify-between bg-blue-900">
                <div className="justify-center flex-col items-center ml-3 space-y-2">
                    <div className="flex items-center space-x-2">
                        <img
                            src={logo}
                            alt="Logo"
                            className="rounded-full w-[50px] h-[50px] bg-slate-700 mt-2"
                        />
                ...
                <div className="flex items-center justify-around w-[70%]">
                    ...
                    <ul className="space-y-4 py-4">{Flist}</ul>
                    <ul className="space-y-4 py-4">{Slist}</ul>
                </div>
            </div>
